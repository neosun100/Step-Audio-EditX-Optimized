...
class WhisperWrapper:
    ...
    def __init__(self, model_id="openai/whisper-large-v3-turbo"):
        ...
        try:
            # Use GPU (Turbo model is 50% smaller than v3, ~1.5GB vs 3GB)
            # Container only has access to GPU 0, but Turbo should fit in remaining VRAM
            self.model = pipeline("automatic-speech-recognition", model=model_id, device="cuda")
            self.logger.info(f"✓ Whisper model loaded successfully: {model_id} (using GPU)")
